...
export function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (data.user) {
      showToast('Account created. Welcome to Cadence.', 'success');
      navigate('/onboarding');
    }
  };
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-sm">
          <Link to="/" className="flex flex-col items-center mb-6">
            <img src="/logo.png" alt="Cadence Logo" className="h-16 w-auto object-contain mb-4" />
          </Link>
          <div className="card p-6">
            <h1 className="font-display text-xl font-semibold text-cadence-text mb-1">Create your account</h1>
            <p className="text-sm text-cadence-muted mb-6">Start with a free plan. No credit card needed.</p>
            {error && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-cadence-muted" />
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="password" className="input pl-10" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} />
                </div>
              </div>
            ...
            <p className="text-sm text-cadence-muted text-center mt-4">
              Already have an account? <Link to="/login" className="text-cadence-accent hover:underline">Log in</Link>
            </p>
          </div>
